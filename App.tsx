...
function AppNavigator() {
    ...
    async function checkBusiness() {
      ...
      try {
        ...
        if (businesses.length > 0) {
          ...
          // If there is a business but no active business is selected,
          // automatically select the first available business.
          if (!business) {
  ...
  if (!hasBusiness) {
    return (
      <BusinessSetupScreen
        onComplete={() => setHasBusiness(true)}
      />
    );
  }

  // Do not enter the main application until the active
  // business has actually been loaded into BusinessContext.
  if (!business) {
    return (
      <View
        style={{
