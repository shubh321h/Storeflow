...
begin
  ...
  values (
    sale_id,
    target_business_id,
    sale_data->>'invoiceNumber',
    customer_id,
    (sale_data->>'subtotal')::numeric,
    (sale_data->>'discount')::numeric,
    (sale_data->>'taxAmount')::numeric,
    (sale_data->>'total')::numeric,
    (sale_data->>'paid')::numeric,
    (sale_data->>'due')::numeric,
    sale_data->>'paymentMethod',
    sale_data->>'status',
    nullif(sale_data->>'notes', ''),
    (sale_data->>'createdAt')::timestamptz
  );
  ...
  loop
    ...
    values (
      ...
      (item_data->>'discount')::numeric,
      (item_data->>'taxRate')::numeric,
      (item_data->>'taxAmount')::numeric,
  ...
  for movement_data in
    select *
    from jsonb_array_elements(payload->'stockMovements')
  loop
    ...
    values (
      (movement_data->>'id')::uuid,
      target_business_id,
      (movement_data->>'productId')::uuid,
      previous_qty,
      change_qty,
      new_qty,
      movement_data->>'type',
      nullif(movement_data->>'reason', ''),
      sale_id,
      (movement_data->>'createdAt')::timestamptz
    );
  ...
  if customer_id is not null
     and (sale_data->>'due')::numeric > 0
  then

    update public.customers as c
    set
      balance = c.balance + (sale_data->>'due')::numeric,
      updated_at = now()
...
as $$
declare
  purchase_data jsonb := payload->'purchase';
...
begin
  ...
  purchase_id := (purchase_data->>'id')::uuid;
  target_business_id := (purchase_data->>'businessId')::uuid;
  supplier_id := nullif(purchase_data->>'supplierId', '')::uuid;


  if purchase_id is null
     or target_business_id is null
  then
    raise exception 'Invalid purchase or business';
  end if;
  ...
  values (
    ...
    (purchase_data->>'taxAmount')::numeric,
    (purchase_data->>'total')::numeric,
    (purchase_data->>'paid')::numeric,
    (purchase_data->>'due')::numeric,
    purchase_data->>'paymentMethod',
    purchase_data->>'status',
    nullif(purchase_data->>'notes', ''),
    nullif(purchase_data->>'supplierInvoiceNumber', ''),
    (purchase_data->>'createdAt')::timestamptz
  );
  ...
  loop
    ...
    values (
      (item_data->>'id')::uuid,
      purchase_id,
      nullif(item_data->>'productId', '')::uuid,
      item_data->>'productName',
      (item_data->>'quantity')::numeric,
      (item_data->>'price')::numeric,
      (item_data->>'discount')::numeric,
  ...
  loop
    ...
    if current_qty <> previous_qty then
      raise exception 'Product stock changed; retry purchase';
    end if;
  ...
  if supplier_id is not null
     and (purchase_data->>'due')::numeric > 0
  then

    update public.suppliers as s
    set
      balance = s.balance + (purchase_data->>'due')::numeric,
      updated_at = now()
    ...
    select
      ...
      (purchase_data->>'createdAt')::timestamptz,
      'purchase',
      'Purchase ' || (purchase_data->>'invoiceNumber'),
      purchase_id,
      (purchase_data->>'due')::numeric,
      0,
      s.balance
...
begin
  ...
  if not found
     or not public.has_business_access(original.business_id)
  then
    raise exception 'Original sale not found or access denied';
  end if;
  ...
  set status = 'returned'
  where id = sale_id
    and business_id = original.business_id;


  for item_data in
    select *
    from jsonb_array_elements(returned_items)
  loop

    qty := (item_data->>'quantity')::numeric;
    ...
    into old_qty
    from public.inventory
    where product_id = (item_data->>'productId')::uuid
      and business_id = original.business_id
    for update;
...
as $$
declare
  original public.purchases%rowtype;
...
begin
  ...
  if not found
     or not public.has_business_access(original.business_id)
  then
    raise exception 'Original purchase not found or access denied';
  end if;
  ...
  loop
    ...
    if old_qty is null or old_qty < qty then
...
begin
  ...
  then
    raise exception 'Invalid payment or access denied';
  end if;
  ...
  values (
    ...
    nullif(payload->>'purchaseId', '')::uuid,
    amount,
    payload->>'method',
  ...
  if customer_id is not null
     and nullif(payload->>'saleId', '') is null
  then
    ...
    where c.id = customer_id
      and c.business_id = target_business_id
    returning c.balance into new_balance;
    ...
    values (
      ...
      customer_id,
      'payment_received',
      'Payment Received - ' || upper(payload->>'method'),
      (payload->>'id')::uuid,
      amount,
      new_balance
    );
  ...
  if supplier_id is not null
     and nullif(payload->>'purchaseId', '') is null
  then
    ...
    where s.id = supplier_id
      and s.business_id = target_business_id
    returning s.balance into new_balance;
